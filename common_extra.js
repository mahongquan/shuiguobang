function _relatedlinks(rlinkmsgid) {
  ...
  var alink_i = 0;
  msg = msg.replace(/(<a.*?<\/a\>)/gi, function ($1) {
    alink[alink_i] = $1;
    alink_i++;
    return '#alink ' + (alink_i - 1) + '#';
  });
  var relatedid = new Array();
  msg = msg.replace(/(^|>)([^<]+)(?=<|$)/gi, function ($1, $2, $3) {
...
function _updatesecqaa(idhash) {
  if ($('secqaa_' + idhash)) {
    ...
    $('checksecqaaverify_' + idhash).innerHTML =
      '<img src="' + IMGDIR + '/none.gif" width="16" height="16" class="vm" />';
    ajaxget(
      ...
      function () {
        secST['qaa_' + idhash] = setTimeout(function () {
          $('secqaa_' + idhash).innerHTML =
            '<span class="xi2 cur1" onclick="updatesecqaa(\'' +
            idhash +
            '\')">刷新验证问答</span>';
        }, 180000);
      }
...
function _updateseccode(idhash, play) {
  if (isUndefined(play)) {
    if ($('seccode_' + idhash)) {
      ...
      $('checkseccodeverify_' + idhash).innerHTML =
        '<img src="' +
        IMGDIR +
      ...
      ajaxget(
        ...
        function () {
          secST['code_' + idhash] = setTimeout(function () {
            $('seccode_' + idhash).innerHTML =
              '<span class="xi2 cur1" onclick="updateseccode(\'' +
              idhash +
              '\')">刷新验证码</span>';
          }, 180000);
        }
    ...
    eval(
      'window.document.seccodeplayer_' + idhash + '.SetVariable("isPlay", "1")'
    );
  }
}
function _checksec(type, idhash, showmsg, recall) {
  ...
  x.loading = '';
  $('checksec' + type + 'verify_' + idhash).innerHTML =
    '<img src="' +
    IMGDIR +
  ...
  x.get(
    'misc.php?mod=sec' +
      ...
      idhash +
      '&secverify=' +
      (BROWSER.ie && document.charset == 'utf-8'
        ? encodeURIComponent(secverify)
        : secverify),
    function (s) {
...
function _showdistrict(
  ...
  var pid =
    changelevel >= 1 && elems[0] && $(elems[0]) ? getdid($(elems[0])) : 0;
  var cid =
    changelevel >= 2 && elems[1] && $(elems[1]) ? getdid($(elems[1])) : 0;
  var did =
    changelevel >= 3 && elems[2] && $(elems[2]) ? getdid($(elems[2])) : 0;
  var coid =
    changelevel >= 4 && elems[3] && $(elems[3]) ? getdid($(elems[3])) : 0;
  var url =
    'home.php?mod=misc&ac=ajax&op=district&container=' +
    container +
    '&containertype=' +
    containertype +
    '&province=' +
...
function _copycode(obj) {
  ...
  setCopy(
    BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent,
    '代码已复制到剪贴板'
  );
}
...
function _zoom(obj, zimg, nocover, pn, showexif) {
  ...
  var loadCheck = function (obj) {
    if (obj.complete) {
      ...
      if (showexif && faid) {
        var x = new Ajax();
        x.get(
          'forum.php?mod=ajax&action=exif&aid=' + faid + '&inajax=1',
          function (s, x) {
            if (s) {
  ...
  var showloading = function (zimg, pn) {
    ...
    $(menuid + '_waiting').style.left =
      (document.body.clientWidth - 42) / 2 + 'px';
    $(menuid + '_waiting').style.top =
      (document.documentElement.clientHeight - 42) / 2 +
  ...
  var showimage = function (zimg, w, h, imgw, imgh) {
    ...
    $(menuid + '_zoomlayer').style.height =
      (mheight < 120 ? 120 : mheight) + 'px';
    $(menuid + '_img').innerHTML =
      '<img id="' +
      zoomid +
      '" src="' +
      zimg +
  ...
  var adjust = function (e, a) {
    ...
    if (!a) {
      ...
      if (BROWSER.ie && BROWSER.ie < 7) {
        ...
        var menuleft = parseFloat(
          menu.getAttribute('left_') || menu.style.left
        );
    ...
    menu.style.width =
      parseInt(ele.width < 300 ? 300 : parseInt(ele.width)) + 20 + 'px';
  ...
  if (!menu && !pn) {
    ...
    if (cover) {
      menu.innerHTML =
        ...
        '<div class="zimg_p" id="' +
        menuid +
        '_picpage"></div><div class="hm" id="' +
        menuid +
        '_img"></div></div>';
    } else {
      menu.innerHTML =
        '<div class="popupmenu_popup" id="' +
        menuid +
  ...
  if (
    typeof zoomgroup == 'object' &&
    zoomgroup[obj.id] &&
    typeof aimgcount == 'object' &&
    aimgcount[zoomgroup[obj.id]]
  ) {
...
function _zoom_page(paid, showexif) {
  var imagesrc = $('aimg_' + paid).getAttribute('zoomfile')
    ? $('aimg_' + paid).getAttribute('zoomfile')
    : $('aimg_' + paid).getAttribute('file');
...
function _switchTab(prefix, current, total, activeclass) {
  ...
  for (var i = 1; i <= total; i++) {
    ...
    $(prefix + '_' + i).className = classname
      .replace(' ' + activeclass + ' ', '')
      .substr(1);
    $(prefix + '_c_' + i).style.display = 'none';
  }
  $(prefix + '_' + current).className =
    $(prefix + '_' + current).className + ' ' + activeclass;
  $(prefix + '_c_' + current).style.display = '';
}
function _initTab(frameId, type) {
  if (
    typeof document['diyform'] == 'object' ||
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (hasClass(arrTab[i], 'frame') || hasClass(arrTab[i], 'tab')) {
      ...
      for (var j in arrTab[i].childNodes) {
        if (
          typeof arrTab[i].childNodes[j] == 'object' &&
      ...
      frameContent.className = hasClass(arrTab[i], 'frame')
        ? 'content cl ' +
          arrTab[i].className.substr(arrTab[i].className.lastIndexOf(' ') + 1)
        : 'content cl';
    ...
    li.innerHTML = arrTab[i]['innerText']
      ? arrTab[i]['innerText']
...
function slideshow(el) {
  ...
  this.slideshows =
    this.slideshows.length > 0 ? this.slideshows[0].childNodes : null;
  ...
  this.slidebardown =
    this.slidebardown.length > 0 ? this.slidebardown[0] : null;
  ...
  if (!this.slidebar) {
    ...
    for (i = 0; i < this.controls.length; i++) {
      if (
        this.slidebarup == this.controls[i] ||
  ...
  this.runRoll = function () {
    for (var i = 0; i < this.slidenum; i++) {
      if (this.slideshows[i] && typeof this.slideshows[i].style != 'undefined')
  ...
  var imgs = this.slideshows.length
    ? this.slideshows[0].parentNode.getElementsByTagName('img')
    : [];
  ...
  this.checkLoad = function () {
    ...
    if (this.imgLoaded < this.imgs.length) {
      ...
      percentEle.innerHTML =
        parseInt((this.imgLoaded / this.imgs.length) * 100) + '%';
      setTimeout(function () {
        obj.checkLoad();
      }, 100);
    } else {
...
function slidexactive(step) {
  ...
  currentslideele =
    hasClass(aim, 'slidebarup') ||
    hasClass(aim, 'slidebardown') ||
    hasClass(parent, 'slidebar')
      ? aim
      : null;
  while (parent && parent != document.body) {
    ...
    if (
      !currentslideele &&
      (hasClass(parent, 'slidebarup') || hasClass(parent, 'slidebardown'))
    ) {
      currentslideele = parent;
    }
  ...
  var children =
    slidebar.length == 0 ? [] : filterTextNode(slidebar[0].childNodes);
  if (
    currentslideele &&
    (hasClass(currentslideele, 'slidebarup') ||
      hasClass(currentslideele, 'slidebardown'))
  ) {
    xactivei = step;
  } else {
    for (var j = 0, i = 0, L = children.length; i < L; i++) {
      ...
      if (
        !hasClass(children[i], 'slidebarup') &&
...
function _showTip(ctrlobj) {
  ...
  if (!$(menuid)) {
    ...
    div.style.display = 'none';
    div.innerHTML =
      '<div class="tip_horn"></div><div class="tip_c">' +
  ...
  if (!$(ctrlid + '_menu')) {
    ...
    for (var i = 0; i < 40; i++) {
      str +=
        ...
        coloroptions[i] +
        '"' +
        (typeof setEditorTip == 'function'
          ? ' onmouseover="setEditorTip(\'' +
            colortexts[i] +
        ...
        (typeof wysiwyg == 'undefined'
          ? "seditor_insertunit('" +
            ...
            tag1 +
            "]')"
          : ctrlid == editorid + '_tbl_param_4'
          ? "$('" + ctrlid + "').value='" + coloroptions[i] + "';hideMenu(2)"
          : "discuzcode('" + tag2 + "', '" + coloroptions[i] + "')") +
        '" title="' +
        colortexts[i] +
        '" />' +
        (i < 39 && (i + 1) % 8 == 0 ? '<br />' : '');
...
function _toggle_collapse(objname, noimg, complex, lang) {
  ...
  if (!noimg) {
    ...
    if (img.tagName != 'IMG') {
      ...
      img.src =
        img.src.indexOf('_yes.gif') == -1
          ? img.src.replace(/_no\.gif/, '_yes.gif')
          : img.src.replace(/_yes\.gif/, '_no.gif');
    }
    img.blur();
  }
  if (complex) {
...
function _extstyle(css) {
  ...
  $('css_extstyle').href = css
    ? css + '/style.css'
    : STATICURL + 'image/common/extstyle_none.css';
  currentextstyle = css;
...
function _showUpgradeinfo() {
  if (!$('g_upmine_menu')) {
    ...
    $('append_parent').appendChild(menu);
    ajaxget(
      'home.php?mod=spacecp&ac=usergroup&showextgroups=1',
      'g_upmine_menu',
      'ajaxwaitid'
    );
  }
...
function _showForummenu(fid) {
  if ($('fjump_menu') && !$('fjump_menu').innerHTML) {
    ajaxget(
      'forum.php?mod=ajax&action=forumjump&jfid=' + fid,
      'fjump_menu',
      'ajaxwaitid'
    );
  }
  ...
  if (BROWSER.ie) {
    if (!isNaN(image.getAttribute('om'))) {
      image.style.marginBottom =
        image.getAttribute('om') +
        (BROWSER.ie < 8 ? 0 : deg == 90 || deg == 270 ? Math.abs(ow - oh) : 0) +
        'px';
    }
    image.style.filter =
      'progid:DXImageTransform.Microsoft.BasicImage(rotation=' + deg / 90 + ')';
  } else {
    switch (deg) {
      case 90:
        var cow = oh,
...
function _createPalette(colorid, id, func) {
  ...
  func = !func ? '' : '|' + func;
  window.frames['c' + colorid + '_frame'].location.href =
    SITEURL +
    STATICURL +
    'image/admincp/getcolor.htm?c' +
    colorid +
    '|' +
    id +
    func;
...
function _setShortcut() {
  $('shortcuttip').onclick = function () {
    ...
    msg +=
      '下载ICO图标</a>' +
      '"，下载完成后，右击桌面快捷文件->属性->更改图标，选择已下载的ICO图标即可';
