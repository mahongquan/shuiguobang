...
function Ajax(recvType, waitId) {
  ...
  aj.post = function (targetUrl, sendString, resultHandle) {
    ...
    aj.XMLHttpRequest.setRequestHeader(
      'Content-Type',
...
function ajaxupdateevent(o) {
  if (typeof o == 'object' && o.getAttribute) {
    if (o.getAttribute('ajaxtarget')) {
      ...
      var ajaxevent = o.getAttribute('ajaxevent')
        ? o.getAttribute('ajaxevent')
        : 'click';
      ...
      _attachEvent(
        ...
        newfunction(
          'ajaxget',
          ajaxurl,
          o.getAttribute('ajaxtarget'),
          o.getAttribute('ajaxwaitid'),
          o.getAttribute('ajaxloading'),
          o.getAttribute('ajaxdisplay')
        )
      );
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
  var waitid =
    typeof waitid == 'undefined' || waitid === null
      ? showid
      : waitid !== ''
      ? waitid
      : '';
  ...
  var handleResult = function () {
    ...
    try {
      ...
      try {
        s = $(ajaxframeid).contentWindow.document.documentElement.firstChild
          .wholeText;
      } catch (e) {
        try {
          s = $(ajaxframeid).contentWindow.document.documentElement.firstChild
...
function ajaxmenu(
  ...
  if (menu) {
    ...
    menu.innerHTML =
      '<div class="' + contentclass + '" id="' + menuid + '_content"></div>';
    $('append_parent').appendChild(menu);
    var url = !isUndefined(ctrlObj.attributes['shref'])
      ? ctrlObj.attributes['shref'].value
      : !isUndefined(ctrlObj.href)
      ? ctrlObj.href
...
function ajaxinnerhtml(showid, s) {
  if (showid.tagName != 'TBODY') {
    ...
    div1.innerHTML =
      '<table><tbody id="' + showid.id + '_tbody">' + s + '</tbody></table>';
...
function loadcss(cssname) {
  if (!CSSLOADED[cssname]) {
    if (!$('css_' + cssname)) {
      ...
      css.rel = 'stylesheet';
      css.href =
        'data/cache/style_' + STYLEID + '_' + cssname + '.css?' + VERHASH;
...
function showMenu(v) {
  ...
  var zindex = isUndefined(v['zindex'])
    ? JSMENU['zIndex']['menu']
    : v['zindex'];
  ...
  if (
    evt == 'click' &&
    in_array(menuid, JSMENU['active'][layer]) &&
    mtype != 'win'
  ) {
  ...
  if (ctrlObj) {
    if (!ctrlObj.getAttribute('initialized')) {
      ...
      ctrlObj.outfunc =
        typeof ctrlObj.onmouseout == 'function' ? ctrlObj.onmouseout : null;
      ...
      ctrlObj.overfunc =
        typeof ctrlObj.onmouseover == 'function' ? ctrlObj.onmouseover : null;
  ...
  if (
    ...
    $(menuid).style.left =
      parseInt($(menuid).style.left) -
      parseInt($('fwin_' + winhandlekey).style.left) +
      'px';
    $(menuid).style.top =
      parseInt($(menuid).style.top) -
      parseInt($('fwin_' + winhandlekey).style.top) +
      'px';
  }
      ...
      for (var i = 0; i < tags.length; i++) {
        ...
        if (_all.length) {
          for (j = 0; j < _all.length; j++) {
            if (
              (!_all[j]['type'] || _all[j]['type'] != 'hidden') &&
              hasshow(_all[j])
            ) {
...
function dragMenu(menuObj, e, op) {
  ...
  if (op == 1) {
    ...
    menuObj.style.left =
      JSMENU['drag'][2] + menudragnow[0] - JSMENU['drag'][0] + 'px';
    menuObj.style.top =
      JSMENU['drag'][3] + menudragnow[1] - JSMENU['drag'][1] + 'px';
...
function setMenuPosition(showid, menuid, pos) {
  ...
  switch (direction) {
    case 0:
      menuObj.style.left =
        (document.body.clientWidth - menuObj.clientWidth) / 2 + 'px';
  ...
  if (!important) {
    ...
    if (in_array(direction, [1, 2]) && mt < 0) {
      ...
      mt + mh > scrollTop + document.documentElement.clientHeight &&
      sy >= mh
    ) {
      mt = bpt - mh;
  ...
  if (direction == 0 && BROWSER.ie && !document.documentElement.clientHeight) {
    menuObj.style.position = 'absolute';
    menuObj.style.top =
      (document.body.clientHeight - menuObj.clientHeight) / 2 + 'px';
  }
...
function showDialog(
  ...
  mode = in_array(mode, ['confirm', 'notice', 'info', 'right'])
    ? mode
    : 'alert';
    ...
    if (parseInt(BROWSER.ie) != 6) {
      loadingst = setTimeout(function () {
        showDialog(
          '',
          'info',
          '<img src="' + IMGDIR + '/loading.gif"> 请稍候...'
        );
      }, 500);
    }
  };
...
function simulateSelect(selectId, widthvalue) {
  ...
  var selectwidth =
    (selectObj.getAttribute('width', i)
      ? selectObj.getAttribute('width', i)
      : widthvalue) + 'px';
  var tabindex = selectObj.getAttribute('tabindex', i)
    ? selectObj.getAttribute('tabindex', i)
    : 1;
...
function initSearchmenu(searchform, cloudSearchUrl) {
  ...
  if (
    typeof cloudSearchUrl == 'undefined' ||
    cloudSearchUrl == null ||
    cloudSearchUrl == ''
  ) {
  ...
  for (var i = 0; i < a.length; i++) {
    if (a[i].className == 'curtype') {
      ...
      formobj.method = 'post';
      if (
        (a[i].rel == 'forum' || a[i].rel == 'curforum') &&
        ...
        if ($('srchFId')) {
          $('srchFId').value =
            a[i].rel == 'forum' ? 0 : a[i].getAttribute('fid');
        }
      } else {
    ...
    a[i].onclick = function () {
      ...
      formobj.method = 'post';
      if (
        (this.rel == 'forum' || this.rel == 'curforum') &&
        defaultUrl != cloudSearchUrl
      ) {
        formobj.action = cloudSearchUrl;
        formobj.method = 'get';
        if ($('srchFId')) {
          $('srchFId').value =
            this.rel == 'forum' ? 0 : this.getAttribute('fid');
        }
      } else {
        formobj.action = defaultUrl;
      }
...
function ipNotice() {
  if ($('ip_notice')) {
    ajaxget(
      'misc.php?mod=patch&action=ipnotice&_r=' + Math.random(),
      'ip_notice',
      ''
    );
  }
...
function noticeTitleFlash() {
  if (
    NOTICETITLE.flashNumber < 5 ||
    (NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State'])
  ) {
    document.title =
      (NOTICETITLE['State'] ? '【　　　】' : '【新提醒】') +
